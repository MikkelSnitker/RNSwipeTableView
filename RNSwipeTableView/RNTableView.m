...
@interface RNTableView()<UITableViewDataSource, UITableViewDelegate, MGSwipeTableCellDelegate> {
...
-(UITableViewCell* )tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (![self hasCustomCells:indexPath.section]){
        ...
        RNTableViewCell * test = (RNTableViewCell*) cell;
        test.delegate = self;
    }
             ...
             swipeSettings:(MGSwipeSettings*) swipeSettings expansionSettings:(MGSwipeExpansionSettings*) expansionSettings
{
    ...
    RNTableViewCell * view = ((RNTableViewCell *)cell);

    CGRect cellFrame = cell.frame;
    
    NSMutableArray<MGSwipeButton*> * result =[NSMutableArray<MGSwipeButton*> array];
    NSArray<CellSwipeButtonsView*> * buttons;
    if(direction == MGSwipeDirectionLeftToRight){
        
        
        expansionSettings.buttonIndex = view.cellView.leftExpansionSettings.buttonIndex;
        expansionSettings.threshold = view.cellView.leftExpansionSettings.threshold;
        expansionSettings.fillOnTrigger = view.cellView.leftExpansionSettings.fillOnTrigger;
        buttons = view.cellView.leftButtons;
       
    } else {
        expansionSettings.buttonIndex = view.cellView.rightExpansionSettings.buttonIndex;
        expansionSettings.threshold = view.cellView.rightExpansionSettings.threshold;
        expansionSettings.fillOnTrigger = view.cellView.rightExpansionSettings.fillOnTrigger;
        buttons = view.cellView.rightButtons;
    }

        for(CellSwipeButtonView * buttonView in buttons){
            UIColor * backgroundColor = buttonView.backgroundColor;
            MGSwipeButton * btn =   [MGSwipeButton buttonWithTitle:nil backgroundColor: backgroundColor padding:15 callback:^BOOL(MGSwipeTableCell * sender){
                
                RNTableViewCell * senderCell = (RNTableViewCell *)sender;

                NSMutableDictionary *newValue = [self dataForRow:senderCell.cellView.row section:senderCell.cellView.section];
                newValue[@"target"] = buttonView.reactTag; // senderCell.cellView.reactTag;
                newValue[@"rowIndex"] = [NSNumber numberWithInteger:senderCell.cellView.row];
                newValue[@"rowSection"] = [NSNumber numberWithInteger:senderCell.cellView.section];
                
                [_eventDispatcher sendInputEventWithName:@"onAction" body:newValue];
                
                return YES;}];
            
            
            [btn setFrame: CGRectMake(0,0, buttonView.componentWidth, cell.frame.size.height)];
            [btn setBounds: CGRectMake(0,0, buttonView.componentWidth, cell.frame.size.height)];
            
            [btn addSubview:buttonView];
            
            [buttonView setFrame:btn.frame];
            [buttonView setBounds:btn.bounds];
            
      
            [buttonView setUserInteractionEnabled:false];
            btn.autoresizesSubviews = YES;
            
            buttonView.autoresizingMask = (UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleWidth);
            buttonView.autoresizesSubviews = YES;
          
            
            [_eventDispatcher.bridge.uiManager setFrame:btn.frame forView:buttonView];
